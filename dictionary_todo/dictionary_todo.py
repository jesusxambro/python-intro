...
def create_todo_item(incoming_todo_list):
    ...
    todo_item["task"] = input("Enter TO DO item: ")
    working_list.append(todo_item)
    print(f"{todo_item["task"]}, was added to the list.")
    return working_list
...
def main():
    ...
    def view_todo_items():
        ...
        for index in range(len(list_of_to_dos)):
            task = list_of_to_dos[index]["task"]
            status = list_of_to_dos[index]["completed"]
            print(f"\nID: {index}. Task: {task} Status: {status}\n")
